...
async function handleSaveDowntime(event) {
    ...
    try {
        ...
        const downtimeData = {
            DowntimeId: document.getElementById(`downtimeSelect${recordHeaderId}`).value,
            ActionTakenId: document.getElementById(`actionTakenSelect${recordHeaderId}`).value,
            TimeStart: timeStart,
            TimeEnd: timeEnd,
            Duration: duration,
            Pic: document.getElementById(`picInput${recordHeaderId}`).value || null,
            RemarksId: document.getElementById(`remarksSelect${recordHeaderId}`).value || null
        };
...
function updateDowntimeBadge(recordHeaderId) {
    ...
    if (badgeContainer) {
        const downtimeSelect = document.getElementById(`downtimeSelect${recordHeaderId}`);
        ...
        badgeContainer.innerHTML = `
            <small class="badge bg-danger text-white me-1 mb-1">
                ${selectedOption ? selectedOption.text.split(' - ')[0] : 'No Downtime'}
            </small>`;
    }
}